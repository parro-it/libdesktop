...
static gboolean quit(gpointer data) {
	if (iteration == stepsIteration)
		stepsQuit = TRUE;
	// TODO run a gtk_main() here just to do the cleanup steps of syncing the clipboard and other
	// stuff gtk_main() does before it returns
	else
		gtk_main_quit();
	return FALSE;
}
...
void uiQueueMain(void (*f)(void *data), void *data) {
	...
	// we have to use g_new0()/g_free() because uiprivAlloc() is only safe to call on the main
	// thread for some reason it didn't affect me, but it did affect krakjoe
